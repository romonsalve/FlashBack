<div class="container-fluid">
        <div class="row-fluid">
          <div class="span12">
		<hr />
	<?php echo $this->TwitterBootstrap->add_crumb("Actividades", null);echo $this->TwitterBootstrap->breadcrumbs(array("divider" => "/")); ?>		<hr />
          <!-- Sheet starts -->
            <div class="box-body">
              <div class="ysheet">
                <div class="bor"></div>
	...
	<?php foreach ($actividades as $actividade): ?>
		...
		<td><?php echo h($actividade['Actividade']['descripcion']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('Ver'), array('action' => 'view', $actividade['Actividade']['id'])); ?>
			<?php echo $this->Html->link(__('Editar'), array('action' => 'edit', $actividade['Actividade']['id'])); ?>
			<?php echo $this->Form->postLink(__('Borrar'), array('action' => 'delete', $actividade['Actividade']['id']), null, __('¿Estás seguro de que quieres eliminar el elemento #%s?', $actividade['Actividade']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</tbody>
	...
	?>
	 ...
	 <?php } ?>		
	 <?php } 
		 else {  echo "<hr /> <h1 style='text-align: center'> Aún no se han ingresado Actividades</h1> <hr />"; } ?>	</div>
	</div>
